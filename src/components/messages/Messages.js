...
const Messages = ({ currentChannel, currentUser, isPrivateChannel }) => {
    ...
    let initialState = {
        ...
        privateMessagesRef: dbRef(db, '/privateMessages/'),
        messagesRef: dbRef(db, '/messages/'),
    };
    ...
    useEffect(() => {
        ...
        const childAdded = onChildAdded(
            query(child(newRefAll, currentChannel.id)),
            (snap) => {
    ...
    const messageSent = (downloadURLImage) => {
        ...
        push(
            child(refFindNew, currentChannel.id),
    ...
    const countUniqueUsers = (someRandomArray) => {
        const uniqueUsers = someRandomArray.reduce((acc, someRandomArray) => {
            if (!acc.includes(someRandomArray.user.id)) {
                acc.push(someRandomArray.user.id);
            }
            return acc;
        }, []);
